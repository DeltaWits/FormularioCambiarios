...
@Component({
  selector: 'app-f2step2',
  standalone: true,
  imports: [FormsModule, ReactiveFormsModule, Tool1Component, NgIf, NgFor, PopUpAlertComponent],

  templateUrl: './f2step2.component.html',
  styleUrl: './f2step2.component.scss'
})
export class F2step2Component {
  @Input() monedas: any = [];
  @Input() formId = ''
  @Input() formF2: any = {
  ...
  checkDvArray(inputNum: number) {
    console.log("entro", this.formF2.identificacion_de_exportador[inputNum].numero_documento)
    if (this.formF2.identificacion_de_exportador[inputNum].numero_documento != '') {
      this.formF2.identificacion_de_exportador[inputNum].dv = this.calculateDV(
        this.formF2.identificacion_de_exportador[inputNum].numero_documento
      );
    }
  ...
  agregarImportador() {
    this.formF2.identificacion_de_exportador.push({
      tipo_documento: '',
      numero_documento: '',
      dv: '',
      nombre_razon_social: ''
  ...
  deleteImportador(index: number) {
    this.formF2.identificacion_de_exportador.splice(
      index,
      1,
    );
  }
